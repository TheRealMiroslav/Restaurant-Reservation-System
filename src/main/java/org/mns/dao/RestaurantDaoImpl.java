...
public class RestaurantDaoImpl implements RestaurantDao {
    ...
    private Restaurant getRestaurants(ResultSet rs) throws SQLException {
        ...
        restaurant.setName(rs.getString("nazev"));
        restaurant.setAddress(rs.getString("adresa"));
        restaurant.setPhoneNumber(rs.getString("telefonniCislo"));
        restaurant.setAverageRating(rs.getDouble("prumerneHodnoceni"));
    ...
    public List<Table> getTablesByRestaurantId(int restauraceId) throws Exception {
        ...
        String sql = "SELECT * FROM Stul WHERE restauraceId = ?";
        ...
        try (Connection conn = DatabaseManager.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                Table s = new Table(rs.getString("kodStolu"), rs.getInt("kapacita"));
    ...
    public void updateRestaurantRating(int restaurantId) throws Exception {
        String sql = "UPDATE Restaurace SET prumerneHodnoceni = " + "(SELECT AVG(hodnoceni) FROM Recenze WHERE restauraceId = ?) " + "WHERE id = ?";
